...
import { logResult, logWarning, green } from '../std';

interface Rule<T = unknown> {
  name: string;
  verify: (pkg: Record<string, unknown>) => T;
  message: (result: T) => void;
}

function isESModule(pkgJson: Record<string, unknown>): boolean {
...
export function inspectPackageConfigs(pkgJson: Record<string, unknown>) {
